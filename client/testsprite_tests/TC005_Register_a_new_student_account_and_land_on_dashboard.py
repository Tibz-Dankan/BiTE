...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /auth/signup and wait for the signup form to render.
        await page.goto("http://localhost:5173/auth/signup")
        
        # -> Fill the signup form (Username, Email, Password, Confirm Password) and click Sign Up to attempt registration.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/div[4]/div/form/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Test Learner')
        
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/div[4]/div/form/div[2]/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('test.learner.20260416+001@example.com')
