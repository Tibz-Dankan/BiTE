...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /auth/signin and wait for the sign-in page UI to load.
        await page.goto("http://localhost:5173/auth/signin")
        
        # -> Fill the email field with the test credential and then the password, then submit the form.
        ...
        # -> Click the 'Sign In' button (index 252) to submit credentials and wait for the app to load the dashboard, then verify dashboard-specific UI appears.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div[4]/div/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
