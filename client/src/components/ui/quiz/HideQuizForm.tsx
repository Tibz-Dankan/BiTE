...
interface HideQuizFormProps {
  quiz: TQuiz;
  onSuccess: (succeeded: boolean) => void;
}

export const HideQuizForm: React.FC<HideQuizFormProps> = (props) => {
  ...
  return (
    ...
    >
      <div className="w-full">
        <h2 className="text-gray-800 font-semibold">Hide Quiz</h2>
      </div>
      <form onSubmit={formik.handleSubmit} className="w-full space-y-6">
        {/* Information text */}
        <div className="w-full p-4 bg-gray-100 rounded-lg border border-gray-300">
          <p className="text-sm text-gray-700">
            Clicking the button below will make the quiz <strong>hidden</strong>{" "}
            from students.
          </p>
        </div>
