...
interface ShowQuestionAIPreviewFormProps {
  questionID: string;
  onSuccess: (succeeded: boolean) => void;
}

export const ShowQuestionAIPreviewForm: React.FC<
  ShowQuestionAIPreviewFormProps
> = (props) => {
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: aiPreviewAPI.showByQuestion,
  ...
  return (
    ...
    >
      <div className="w-full">
        <h2 className="text-gray-800 font-semibold">Show AI Preview</h2>
      </div>
      <form onSubmit={formik.handleSubmit} className="w-full space-y-6">
        <div className="w-full p-4 bg-gray-100 rounded-lg border border-gray-300">
          <p className="text-sm text-gray-700">
            Clicking the button below will make the AI preview{" "}
            <strong>visible</strong> for this question.
          </p>
        </div>
