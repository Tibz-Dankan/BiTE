...
interface HideQuestionAIPreviewFormProps {
  questionID: string;
  onSuccess: (succeeded: boolean) => void;
}

export const HideQuestionAIPreviewForm: React.FC<
  HideQuestionAIPreviewFormProps
> = (props) => {
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: aiPreviewAPI.hideByQuestion,
  ...
  return (
    ...
    >
      <div className="w-full">
        <h2 className="text-gray-800 font-semibold">Hide AI Preview</h2>
      </div>
      <form onSubmit={formik.handleSubmit} className="w-full space-y-6">
        <div className="w-full p-4 bg-gray-100 rounded-lg border border-gray-300">
          <p className="text-sm text-gray-700">
            ...
            <strong>hidden</strong> for this question.
          </p>
        </div>
