...
interface ShowAllAIPreviewsFormProps {
  quizID: string;
  onSuccess: (succeeded: boolean) => void;
}

export const ShowAllAIPreviewsForm: React.FC<ShowAllAIPreviewsFormProps> = (
  props,
) => {
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: aiPreviewAPI.showAllByQuiz,
  ...
  return (
    ...
    >
      <div className="w-full">
        <h2 className="text-gray-800 font-semibold">Show All AI Previews</h2>
      </div>
      <form onSubmit={formik.handleSubmit} className="w-full space-y-6">
        <div className="w-full p-4 bg-gray-100 rounded-lg border border-gray-300">
          <p className="text-sm text-gray-700">
            ...
            <strong>visible</strong> for all questions in this quiz.
          </p>
        </div>
