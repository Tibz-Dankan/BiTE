...
interface MakeDefaultAIPreviewFormProps {
  aiPreviewID: string;
  onSuccess: (succeeded: boolean) => void;
}

export const MakeDefaultAIPreviewForm: React.FC<
  MakeDefaultAIPreviewFormProps
> = (props) => {
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: aiPreviewAPI.makeDefault,
  ...
  return (
    ...
    >
      <div className="w-full">
        <h2 className="text-gray-800 font-semibold">Make Default AI Preview</h2>
      </div>
      <form onSubmit={formik.handleSubmit} className="w-full space-y-6">
        <div className="w-full p-4 bg-gray-100 rounded-lg border border-gray-300">
          <p className="text-sm text-gray-700">
            Clicking the button below will set this AI preview as the{" "}
            <strong>default</strong> preview for this question. The current
