...
export const DeleteAnswer: React.FC<DeleteAnswerProps> = ({
  answerID,
  answerTitle,
  onSuccess,
  onCancel,
  ...
  const { isPending, mutate: deleteAnswer } = useMutation({
  ...
  return (
    <div className="w-full flex flex-col gap-4 p-4">
      <div className="flex items-center gap-2 text-red-500">
        ...
        <h3 className="font-semibold text-lg">Delete Answer</h3>
      </div>
      <p className="text-sm text-gray-600">
        ...
        {answerTitle ? (
          <>
            {" "}
            <span className="font-bold">"{answerTitle}"</span>?
          </>
        ) : (
          " this answer?"
        )}
